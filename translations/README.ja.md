...
- **層数 `D`**：十分位数型の推定量は層数の選択を必要とします。連続的で帯域幅制御された勾配推定量（十分位数推定量はその厳密なボックスカー特殊例です）は実装され検証されましたが、外部検査により、それは十分位数よりもノイズが多く、`D`感度を低減しないことが示されました（操作変数標準化勾配は比であり、平滑化を細かくするとその分母が縮小します）。推奨される手法は、連続的な再パラメータ化ではなく、`D`にわたる感度分析を伴う離散推定量です（`dev/wp15-continuous-isg.md`を参照）。

### パフォーマンスと推論バックエンド（オプトイン）

デフォルトのバックエンドは変更されていませんが、第II期の作業（`dev/acceleration-roadmap.md`）により、`mrwin_controls()`を通じて検証済みのオプトイン代替手段が追加されました。
