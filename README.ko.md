...
다음을 포함한 형식화된 보고서 생성:
- 주요 DS-CWR 추정치, CI, p-값, 선택적 다면발현 경계 CI
...
- **계산 확장성(개발 중)**: 현재 win/loss 쌍별 스윕은 부트스트랩 반복마다
  N에 대해 2차이므로, 바이오뱅크 규모는 통계적 전제조건이지만 아직 계산상의
  기본값은 아닙니다. 준2차(거의 N log N) 백엔드, 임의의 층 수를 제거하는 연속
  기울기 추정량, 해석적 분산이 2단계 로드맵(`inst/spec/acceleration-roadmap.md`)에
  명시되어 있으며, 모두 기존 결과를 바꾸지 않는 선택적 백엔드로 추가됩니다.
