...
- **分层数 `D`**：十分位数式估计量需要选择分层数。一个连续的、带宽控制的梯度估计量（十分位数估计量是其精确的 boxcar 特例）已被实现并验证，但外部检查表明它比十分位数更嘈杂，且不会降低对 `D` 的敏感性（工具标准化梯度是一个比值，更精细的平滑会使其分母收缩）。推荐做法是采用离散估计量并对 `D` 进行敏感性分析，而非连续重参数化（参见 `dev/wp15-continuous-isg.md`）。

### 性能与推断后端（可选启用）

默认后端保持不变，但第二阶段工作（`dev/acceleration-roadmap.md`）通过 `mrwin_controls()` 添加了经过验证的可选替代方案：
