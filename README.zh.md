...
生成格式化的报告，包含：
- 主要 DS-CWR 估计值、主要（Fieller）CI、p 值，以及可选的多效性约束 CI
- 相邻层梯度（log-theta、Delta-X、ISG、CWR）
- 异质性 Q 统计量
- 作为带标签参考的 delta 方法区间（Fieller 区间为主要区间；参见下文*置信区间*）
- SDPD 诊断（启用时）
- 结构化警告
...
- **分层数 `D`**：十分位数式估计量需要选择分层数。一个连续的、带宽控制的梯度估计量（十分位数估计量是其精确的 boxcar 特例）已被实现并验证，但外部检查表明它比十分位数更嘈杂，且不会降低对 `D` 的敏感性（工具标准化梯度是一个比值，更精细的平滑会使其分母收缩）。推荐做法是采用离散估计量并对 `D` 进行敏感性分析，而非连续重参数化（参见 `inst/spec/wp15-continuous-isg.md`）。

### 性能与推断后端（可选启用）

默认后端保持不变，但第二阶段工作（`inst/spec/acceleration-roadmap.md`）通过 `mrwin_controls()` 添加了经过验证的可选替代方案：
...
**主要报告的区间是 Fieller 区间。**外部校准模拟表明，原始的 delta 方法比值区间过宽（未达到名义第一类错误），而基于相同协方差的 Fieller 构造则得到正确校准；delta 方法区间仅作为带标签的参考保留。在弱工具下，Fieller 区间被诚实地报告为无界，而非一个虚假狭窄的集合。
