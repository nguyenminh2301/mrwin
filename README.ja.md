...
- 主要なDS-CWR推定値、主要（Fieller）CI、p値、およびオプションの多面発現境界CI
- 隣接層勾配（log-theta、Delta-X、ISG、CWR）
- 異質性Q統計量
- ラベル付き参照としてのデルタ法区間（Fieller区間が主要なものです。下記の*信頼区間*を参照）
- SDPD診断（有効な場合）
- 構造化された警告
...
- **層数 `D`**：十分位数型の推定量は層数の選択を必要とします。連続的で帯域幅制御された勾配推定量（十分位数推定量はその厳密なボックスカー特殊例です）は実装され検証されましたが、外部検査により、それは十分位数よりもノイズが多く、`D`感度を低減しないことが示されました（操作変数標準化勾配は比であり、平滑化を細かくするとその分母が縮小します）。推奨される手法は、連続的な再パラメータ化ではなく、`D`にわたる感度分析を伴う離散推定量です（`inst/spec/wp15-continuous-isg.md`を参照）。

### パフォーマンスと推論バックエンド（オプトイン）

デフォルトのバックエンドは変更されていませんが、第II期の作業（`inst/spec/acceleration-roadmap.md`）により、`mrwin_controls()`を通じて検証済みのオプトイン代替手段が追加されました。

- `backend = "fast"` — 準2次の、コンパイルされた（Rcpp）win/lossカーネル（`K`個の優先度レベルに対して`Theta(N log^{K-1} N)`）であり、密バックエンドとビット単位で同一であり、推定量をバイオバンク規模に到達させます（例：`K = 3`、`N = 80,000`を1秒未満）。
- `inference = "analytic"` — マルチプライヤーブートストラップを再現する閉形式の影響関数分散（GWAS重みの不確実性に対する厳密なモンテカルロ項を伴う）であり、サンプリング成分のためのブートストラップループを排除します。

> **二重順位付け層別化 — 推奨されません（否定的な知見）。**
> `mrwin_doubly_ranked_strata()`（Tian/Burgess）は実装されていますが、外部較正
> により、`stratification = "doubly_ranked"`は層間DS-CWR推定対象と**非互換**である
> ことが示されました。これは操作変数を層間で均衡させるため、隣接層の対比が交絡
> 駆動になります（交絡下で第I種過誤 ~1.0）。`mrwin()`は依然としてこれを実行します
> が、`doubly_ranked_invalid`警告を発します。デフォルトは`"prs_rank"`です。
> `inst/spec/validation-findings.md`を参照してください。

#### 信頼区間

**主要に報告される区間はFieller区間です。**外部較正シミュレーションにより、元のデルタ法比区間が広すぎる（名目第I種過誤に達しない）一方で、同じ共分散上でのFieller構成は正しく較正されることが示されました。デルタ法区間はラベル付き参照としてのみ保持されます。弱い操作変数の下では、Fieller区間は誤って狭い集合としてではなく、正直に非有界として報告されます。
